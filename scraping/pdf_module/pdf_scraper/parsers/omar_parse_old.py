...
def get_all(filepath: str, xml_tree: ET.ElementTree) -> dict:
    ...
    omar = {
        "pdf_file": xpu.file_get_name_pdf(xml_header)
    }
...
def get_conditions_number(xml_data: ET.Element) -> int:
    ...
    """
    count = 0
    for p in xpu.get_paragraphs_by_header("introductory comment", xml_data):
        if "•" in p:
            count += 1
...
def get_prevalence(xml_data: ET.Element) -> str:
    ...
    """
    for p in xpu.get_paragraphs_by_header("comp position adopted", xml_data):
...
def get_prevalence_section(section: ET.Element) -> str:
    if xpu.section_contains_header_substring("comp position adopted", section):
        section_text = xpu.section_append_paragraphs(section)
        for p in section_text:
            # Find the paragraph with the bullet points
            if "•" in p:
                bullets = p.split("•")
                # Find the bullet point with the prevalence data in it
