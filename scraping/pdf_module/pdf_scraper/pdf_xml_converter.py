...
def remove_illegal_characters(string: str) -> str:
    ...
    for character in string:
        ...
        if encoded_char >= 0x1 and encoded_char <= 0x8:
            is_illegal = True

        if encoded_char >= 0xB and encoded_char <= 0xC:
            is_illegal = True

        if encoded_char >= 0xE and encoded_char <= 0x1F:
            is_illegal = True

        if encoded_char >= 0x7F and encoded_char <= 0x84:
            is_illegal = True

        if encoded_char >= 0x86 and encoded_char <= 0x9F:
            is_illegal = True

        if encoded_char in [0x00, 0xEFFF, 0xEFFF]:
            is_illegal = True
