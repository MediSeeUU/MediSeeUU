...
class ScraperAttribute:
    def __init__(self, name: str, sources: list[str], combine=False):
        self.name = name
...
def AttributeFactory(all_attributes: dict[str, ScraperAttribute], name: str, sources: list[str], combine=False):
    attr = ScraperAttribute(name, [web])
...
eu_aut_status = AttributeFactory(all_attributes, "eu_aut_status", [web, web])  # niet web web pls
