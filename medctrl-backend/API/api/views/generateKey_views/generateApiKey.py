...
class GenerateKeyForm(forms.Form):
    user = forms.ModelChoiceField(queryset=User.objects.filter(is_superuser=0).filter(is_staff=0).filter(~Q(username='AnonymousUser')))
...
class GenerateKeyView(FormView):
    ...
    def form_valid(self, form):
        ...
        instance, token = AuthToken.objects.create(user, datetime.timedelta(days=duration))
