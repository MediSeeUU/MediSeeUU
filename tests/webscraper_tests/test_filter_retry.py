...
class TestFilterRetry(unittest.TestCase):
    ...
    @parameterized.expand([
        [
            b"EU-1-00-000_o_w_dec_0.pdf@https://ec.europa.eu/health/documents/community-register/2005/2005122310805/dec_10805_en.pdf@corrupt@EU/3/05/339@no_pdf_json_found",
            True],
        [
            b"EU-3-05-339_o_w_dec_0.pdf@https://ec.europa.eu/health/documents/community-register/2005/2005122310805/dec_10805_en.pdf@corrupt@EU/3/05/339@no_pdf_json_found",
            False]
    ])
    def test_retry_all(self, file_content, err_check):
        url_file = {"EU-3-05-339": {"ec_url": "https://ec.europa.eu/health/documents/community-register/html/o339.htm",
                                    "aut_url": [
                                        "https://ec.europa.eu/health/documents/community-register/2005/2005122310805/dec_10805_en.pdf"],
                                    "smpc_url": [],
                                    "ema_url": [
        ...
        if err_check:
            with self.assertRaises(KeyError) as context:
                filter_retry.retry_all(temp.name, url_file)
            self.assertTrue('EU-1-00-000' in str(context.exception))
        else:
            self.assertIsNone(filter_retry.retry_all(temp.name, url_file))

    @parameterized.expand([
        ["EU-1-04-273", ["h", "a", "anx", "0"]],  # normal
        ["EU-1-04-273", ["h", "a", "an", "0"]],  # typo in pdf type
    ...
    def test_retry_download(self, eu_n, filename_el):
        url_file = {'EU-1-00-129': {'ec_url': 'https://ec.europa.eu/health/documents/community-register/html/h129.htm',
                                    'aut_url': [''],
                                    'smpc_url': [
                                        'https://ec.europa.eu/health/documents/community-register/2000/200003093513/anx_3513_en.pdf'],
                                    },
                    'EU-1-04-273': {'ec_url': 'https://ec.europa.eu/health/documents/community-register/html/h274.htm',
                                    'aut_url': [
                                        'https://ec.europa.eu/health/documents/community-register/2004/200404267651/dec_7651_en.pdf'],
                                    'smpc_url': [
                                        'https://ec.europa.eu/health/documents/community-register/2004/200404267651/dec_7651_en.pdf'],
                                    },
