...
def getURLsForPDFAndEMA(url: str):
    ...
    plaintext_json = re.findall(r"(?<=var .+?)\[.+?\](?=;)", scriptTag.text, re.DOTALL)
    for match in plaintext_json: match = match.replace("\r", "").replace("\n", "") # Remove newlines
        
    #get the url linking to the EMA website, where the EPAR + OMAR files can be found. Sometimes there are two URLS to the EMA website.
    ...
    for row in procedures_json:
        if row["files_dec"] == None and row["files_anx"] == None:
            continue
