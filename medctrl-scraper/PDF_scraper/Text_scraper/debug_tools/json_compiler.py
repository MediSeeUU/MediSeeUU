...
def compile_json_files(directory: str):
    subdirectories = [subdirectory for subdirectory in listdir(directory) if path.isdir(path.join(directory, subdirectory))]
...
def get_medicine_json(medicine_path: str, json_list: list):
    medicine_jsons = [file for file in listdir(medicine_path) if path.isfile(path.join(medicine_path, file)) and "pdf_parser.json" in file]
