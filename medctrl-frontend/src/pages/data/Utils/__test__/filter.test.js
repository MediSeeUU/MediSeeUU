...
test('single number filter applied correctly', () => {
  let filteredData = filterData(DummyData, [
    { selected: 'ApplicationNo', input: [{var: '8', filterRange: 'from'}], filterType: 'number' },
  ])
  filteredData.forEach((element) => {
...
test('two number filters applied correctly', () => {
  let filteredData = filterData(DummyData, [
    { selected: 'ApplicationNo', input: [{var: '2000', filterRange: 'from'}], filterType: 'number' },
    { selected: 'DecisionYear', input: [{var: '2001', filterRange: 'till'}], filterType: 'number' },
  ])
  filteredData.forEach((element) => {
...
test('multiple values in number filter applied correctly', () => {
  let filteredData = filterData(DummyData, [
    { selected: 'DecisionYear', input: [{var: '1997', filterRange: 'from'}, {var: '2001', filterRange: 'till'}], filterType: 'number' },
  ])
  filteredData.forEach((element) => {
...
test('single date filter applied correctly', () => {
  let filteredData = filterData(DummyData, [
    { selected: 'DecisionDate', input: [{var: '2012-03-20', filterRange: 'from'}], filterType: 'date' },
  ])
  filteredData.forEach((element) => {
    const elDate = new Date(element.DecisionDate)
    expect(elDate.getTime()).toBeGreaterThanOrEqual(new Date("2012-03-20").getTime())
  })
})

test('two date filters applied correctly', () => {
  let filteredData = filterData(DummyData, [
    { selected: 'DecisionDate', input: [{var: '2000-03-20', filterRange: 'from'}], filterType: 'date' },
    { selected: 'DecisionDate', input: [{var: '2012-03-20', filterRange: 'till'}], filterType: 'date' },
  ])
  filteredData.forEach((element) => {
    const elDate = new Date(element.DecisionDate)
    expect(elDate.getTime()).toBeGreaterThanOrEqual(new Date("2000-03-20").getTime())
    expect(elDate.getTime()).toBeLessThanOrEqual(new Date("2012-03-20").getTime())
  })
})

test('multiple values in date filter applied correctly', () => {
  let filteredData = filterData(DummyData, [
    { selected: 'DecisionDate', input: [{var: '2000-03-20', filterRange: 'from'}, {var: '2012-03-20', filterRange: 'till'}], filterType: 'text' },
  ])
  filteredData.forEach((element) => {
    const elDate = new Date(element.DecisionDate)
    expect(elDate.getTime()).toBeGreaterThanOrEqual(new Date("2000-03-20").getTime())
...
test('multiple values in bool filter applied correctly', () => {
  let filteredData = filterData(DummyData, [
    { selected: 'ATMP', input: [{var: 'yes'}, {var: 'no'}], filterType: 'bool' },
  ])
  ...
  filteredData.forEach((element) => {
    expect(element.ATMP.toString() === 'yes' || element.ATMP.toString() === 'no').toBeTruthy()
